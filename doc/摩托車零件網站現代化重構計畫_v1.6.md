...
> **版本**：v1.6（實作 B2B 詢價車、後台 CRM、郵件通知與拖曳排序功能）
...
| **v1.5** | **正式網域定案為 `xh-motorparts.com`，更新程式碼（robots.ts/sitemap.ts/site.ts）與環境變數範本中的 Canonical URL、R2、Resend 配置；規畫 GitHub 專案移轉朋友帳號策略與專案名稱建議** |
| **v1.5.1** | **策略調整為 Next.js 單軌開發；不再以舊版靜態站作為維運主線，所有後續功能以 `src/app/` 架構為準。** |
| **v1.6** | **實作後台大分類與子目錄拖曳排序功能，重構並修飾三語隱私政策排版，新增前台 B2B RFQ 詢價購物車、後台 CRM 詢價管理、與 Resend 郵件雙向自動通知** |
...
export async function generateMetadata({ params }: { params: { locale: string } }) {
  const baseUrl = 'https://xh-motorparts.com'
...
**產生的 HTML**：
```html
<link rel="canonical"              href="https://xh-motorparts.com/zh-TW/products" />
<link rel="alternate" hreflang="zh-TW" href="https://xh-motorparts.com/zh-TW/products" />
<link rel="alternate" hreflang="zh-CN" href="https://xh-motorparts.com/zh-CN/products" />
<link rel="alternate" hreflang="en"    href="https://xh-motorparts.com/en/products" />
<link rel="alternate" hreflang="x-default" href="https://xh-motorparts.com/en/products" />
```
...
export default function robots(): MetadataRoute.Robots {
  return {
    ...
    sitemap: 'https://xh-motorparts.com/sitemap.xml',
  }
}
```
...
@media print {
  ...
  /* 頁尾浮水印 */
  body::after {
    content: "XH Motor Parts | xh-motorparts.com";
    position: fixed;
...
| **錯誤監控** | **Sentry** | 免費 5,000 次錯誤/月，14天資料保留 | **$0 / 月** |
| **Uptime 監控** | **UptimeRobot** | 免費 50 個監控點，5分鐘一次 | **$0 / 月** |
| **網域 (Domain)** | **Namecheap** | 自訂網域（確案為 `xh-motorparts.com`） | 約 **$12/年 (~$380台幣)** |
...
**確案網址**：`xh-motorparts.com`

*   **優勢**：結合 XH（協煌）公司品牌縮寫與 Motor Parts（摩托車零件）業務核心，簡明好記，並具有強烈的品牌專業度，利於全球推廣。

> **決策更新**：已在重構計畫 v1.5 中將所有設定配置預設更新為 `xh-motorparts.com`。
...
| **Resend** | 朋友公司 | — | 用公司網域信箱驗證（如 `admin@xh-motorparts.com`）|
...
```
Step 1：朋友購買網域（如 xh-motorparts.com）
           ↓
Step 2：用網域建立公司專用 Email（如 admin@xh-motorparts.com）
...
> **推薦**：先用 **Cloudflare Email Routing**（零費用）將 `admin@xh-motorparts.com` 轉發到朋友的個人 Gmail，待業務成長後再升級 Google Workspace。
...
R2_PUBLIC_URL=https://images.xh-motorparts.com
...
RESEND_FROM_EMAIL=noreply@xh-motorparts.com
RESEND_ADMIN_EMAIL=admin@xh-motorparts.com
...
# 網站設定
NEXT_PUBLIC_BASE_URL=https://xh-motorparts.com
...
- `https://xh-motorparts.com/api/health`（每 5 分鐘）
- `https://xh-motorparts.com`（首頁，每 5 分鐘）
